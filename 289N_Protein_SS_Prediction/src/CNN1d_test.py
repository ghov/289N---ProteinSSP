...
reshaped_input_dimension = (700, 22)
...
model.add(Conv1D(filters=9, kernel_size=1, strides=1, activation='relu', input_shape=(reshaped_input_dimension)))
...
my_history = model.fit(X_Train[0:5000, :, :], Y_Train[0:5000, :, :], epochs=160, verbose=1, batch_size=1000, validation_split=0.2)

loss_history = np.array(my_history.history['loss'])

model.save(this_directory_path + 'cnn1d_model_val_' + str(np.amin(loss_history)) +'.h5')
